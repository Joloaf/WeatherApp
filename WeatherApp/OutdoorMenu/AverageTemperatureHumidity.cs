    ...
    internal class AverageTemperatureHumidity
    {
        public static void ShowAverageTempHumidity()
        {
            ...
            while (true)
            {
                ...
                // Hitta medeltemperatur och luftfuktighet för utomhusdata
                var selectedDateData = weatherData
                    .Where(w => $"{w.Year}-{w.Month}-{w.Day}" == inputDate && w.Location.Equals("ute", StringComparison.OrdinalIgnoreCase))
                ...
                // Visa resultat eller felmeddelande
                if (selectedDateData == null)
                {
                    ShowErrorMessage("No outdoor temperature data found for this date!");
                }
                else
                {
        ...
        private static void ShowInputPanel(string inputDate)
        {
            ...
            var inputPanel = new Panel($"[bold]Enter a date (YYYY-MM-DD) for outdoor temperature:[/] [cyan]{inputDate}[/]")
        ...
        private static void ShowResult(dynamic selectedDateData)
        {
            var resultPanel = new Panel($"[bold]Date:[/] {selectedDateData.Date}\n" +
                                        $"[bold]Avg Outdoor Temp:[/] {selectedDateData.AverageTemperature:F1}°C\n" +
                                        $"[bold]Avg Humidity:[/] {selectedDateData.AverageHumidity:F1}%")
